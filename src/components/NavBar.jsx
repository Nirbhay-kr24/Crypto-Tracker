...
const NavBar = () => {
    ...
    const inputHandler =(event) => {
        ...
        if(value === ""){
            ...
            const suggestions = cryptoList.filter((coins) => coins.name.toLowerCase().includes(value.toLowerCase()))
    ...
    return (
        ...
        >
          ...
          <form
            ...
            className="order-2 w-full md:order-3 md:w-auto flex-1 max-w-md mx-0 md:ml-auto relative"
          >
            <div className="relative group">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-600/40 to-cyan-500/40 rounded-full blur opacity-50 transition duration-300" />
  ...
  <ul className="absolute w-full bg-gray-800/95 border border-gray-700 shadow-xl z-10 backdrop-blur-md rounded-lg mt-2">
    {filteredCoins.map((coin, idx) => (
        <li
        key={idx} className="px-4 py-2 hover:bg-emerald-600/30 cursor-pointer text-gray-100"
        onClick={() => {
          setInput(coin.name);
          setFilteredCoins([]);
        }}>
